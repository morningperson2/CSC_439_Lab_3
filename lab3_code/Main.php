...
class ChallengeModel {
    ...
    */
    ...
    public function get_succeed_id() : string{
...
class AdventureController {
    ...
    private array $challenge_database;
    
    public function __construct(CharacterModel $character, array $database){
        $bro = new BinaryRandomOracle(new RNG());
        $this->character = $character;
        $this->challenge_database = $database;
    }
    
    public function get_character() : CharacterModel{
        return $this->character;
    }
    
    public function get_challenge_database() : array{
        return $this->challenge_database;
    }
    
    public function add_challenge(ChallengeModel $challenge) : void{
        if(count($this->challenge_database) >= 5){
            return;
        }
        if($challenge->get_id() == "_end"){
            throw new InvalidArgumentException("Invalid ID");
        }
        if(array_key_exists($challenge->get_id(), $this->challenge_database))
    ...
    public function remove_challenge(string $ID = null) : void{
        if(empty($this->challenge_database)){
            throw new InvalidArgumentException("The challenge database is empty");
        }
        if(!$this->challenge_database[$ID] && $ID != null){
            throw new InvalidArgumentException("ID does not exist");
        }
        if($ID == null){
            unset($this->challenge_database);
            $this->challenge_database = array();
        }
        else{
            unset($this->challenge_database[$ID]);
        }
    ...
    public function validate_challenge() : bool{
        ...
        foreach($this->challenge_database as $challenge){
            ...
            if($challenge->get_succeed_id() == "_end"){
                $has_end = TRUE;
            }
